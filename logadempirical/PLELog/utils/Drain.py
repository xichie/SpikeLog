...
class Para:
    def __init__(self, rex=None, path='./', depth=4, st=0.4, maxChild=100, logName='rawlog.log', removable=True,
...
class Drain:
    ...
    def mainProcess(self):
        ...
        with open(self.para.path + self.para.logName, encoding='utf8') as lines:
            count = 0
            for line in lines:
...
if __name__ == '__main__':
    ...
    path = '../dataset/Spirit/'
    fileName = 'spirit_10k.log'
    removeCol = [0, 1, 2, 3, 4, 5, 6, 7]  # [0,1,2,3,4] for HDFS
    st = 0.5
    depth = 4
    rex = [r'(\d+\.){3}\d+', r'(\/.*?\.[\S:]+)', r'(/[a-zA-Z0-9_-]+)+',
                  r'((?<=[^A-Za-z0-9])|^)([\-\+]?\d+)((?=[^A-Za-z0-9])|$)',
                  r'((?<=[^A-Za-z0-9])|^)(0x[a-f0-9A-F]+)((?=[^A-Za-z0-9])|$)']

    parserPara = Para(savePath='../dataset/Spirit/results/', path=path, st=st, logName=fileName, removeCol=removeCol,
                      rex=rex,
                      depth=depth)
