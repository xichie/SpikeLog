...
class DualInputNet(torch.nn.Module):
    def __init__(self, n_features, hidden_dims=100, n_layers=2,  rep_dim=64,
                 activation='ReLU', bias=False):
        super(DualInputNet, self).__init__()

        encoder_layer = nn.TransformerEncoderLayer(d_model=n_features, nhead=6, batch_first=True)
        self.enc_net = nn.TransformerEncoder(encoder_layer, num_layers=3)
    ...
    def forward(self, x1, x2):
        x1 = self.enc_net(x1)
        x2 = self.enc_net(x2)
        pred = self.out_layer(torch.cat([x1[:,-1], x2[:, -1]], dim=1))
        return pred
