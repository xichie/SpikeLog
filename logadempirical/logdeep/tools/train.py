...
from logadempirical.logdeep.dataset.log import log_dataset, pairwise_log_dataset, pairwise_log_dataset_for_test
...
from logadempirical.logdeep.models.lstm import deeplog, loganomaly, robustlog, DualInputNet
from logadempirical.logdeep.models.autoencoder import AutoEncoder
from logadempirical.logdeep.models.cnn import TextCNN
# from logadempirical.logdeep.models.spiknet import DualInputNet_Spike
...
class Trainer():
    def __init__(self, options):
        ...
        if self.model_name in ["cnn", "logrobust", "autoencoder", "neurallog", "prolog"]:
        ...
        if self.sample == 'sliding_window':
            print("Loading train dataset {}\n".format(self.data_dir + "train.pkl"))
            data = load_features(self.data_dir + "train.pkl",
                                 only_normal=self.is_predict_logkey)
            n_train = int(len(data) * self.train_size)
            
            train_logs, train_labels = sliding_window(data[:n_train],
                                                      vocab=vocab,
                                                      ...
                                                      in_size=self.embedding_dim  # loganomaly 
                                                      )

            train_logs, train_labels = shuffle(train_logs, train_labels)
       
            
            # print('Loading test dataset {}....'.format(self.data_dir + "test.pkl"))
            # test_data = load_features(self.data_dir + "test.pkl", only_normal=self.is_predict_logkey)
            # test_logs, test_labels = sliding_window(test_data,
            #                                 vocab=vocab,
            #                                 window_size=self.history_size,
            #                                 data_dir=self.emb_dir,
            #                                 is_predict_logkey=self.is_predict_logkey,
        ...
        # print('Sequence length: ', len(train_logs[0][0][-1]))
        # print('Embedding dimension:', len(train_logs[0][0][-1][0]))
        if self.model_name == 'prolog':
            train_dataset = pairwise_log_dataset(logs=train_logs,
                                        labels=train_labels)
            valid_dataset = pairwise_log_dataset_for_test(test_logs=val_logs,
                                        test_labels=val_labels, train_logs=train_logs, train_labels=train_labels)
            
            # test_dataset = pairwise_log_dataset_for_test(test_logs=test_logs,
            #                                 test_labels=test_labels, train_logs=train_logs, train_labels=train_labels)
            ...
            valid_dataset = log_dataset(logs=val_logs,
                                        labels=val_labels)
            # test_dataset = log_dataset(test_logs, test_labels)
        ...
        elif self.model_name == 'prolog':
            # self.model = DualInputNet(n_features=self.embedding_dim, rep_dim=self.embedding_dim).to(self.device)
            self.model = DualInputNet_Spike(num_inputs=self.embedding_dim, num_hidden=self.hidden_size, num_out=32).to(self.device)
        else:
        ...
        for i, (log, label) in enumerate(tbar):
            with torch.no_grad():
                ...
                if isinstance(output, dict):
                    ...
                    if self.is_predict_logkey:
                        label = label - 1 # for deeplog and loganomaly
                    label = label.view(-1).to(self.device)
    ...
    def train_prolog(self, epoch):
        ...
        num_batch = len(self.train_loader)
        total_losses = 0
        for i, (pair_log, label) in enumerate(tbar):
            x1 = pair_log[0]['features'][2].to(self.device)
            x2 = pair_log[1]['features'][2].to(self.device)
            label = label.to(self.device)
                
            output = self.model(x1, x2).flatten()
            
            loss = self.mae_criterion(output, label)
            total_losses += float(loss)
    ...
    def valid_prolog(self, epoch):
        ...
        scores = []
        labels = []
        for i, (inputs, label) in enumerate(tbar):
            x1, x2_a_list, x2_u_list = inputs
            label = label.to(self.device)
            with torch.no_grad():
                output = torch.zeros((len(x1['features'][2]))).to(self.device)
                input1 = x1['features'][2].to(self.device)
                for j in range(len(x2_a_list)):
                    x2_a = x2_a_list[j]
                    x2_u = x2_u_list[j]
                    input2 = x2_a['features'][2].to(self.device)
                    output1 = self.model(input1, input2).flatten()
                    input3 = x2_u['features'][2].to(self.device)
                    output2 = self.model(input1, input3).flatten()
                    output += output1 + output2
                output = output / len(x2_a_list) / 2
                
                loss = self.mae_criterion(output, label)   
                total_losses += float(loss)
                scores.extend(output.cpu().squeeze().tolist())
                label = np.array([y.cpu() for y in label])
                labels.extend(label)
                total_log += len(label)
                
        metr = cal_metrics(labels, scores) 
        if total_log:
            print("\nValidation loss:", total_losses / num_batch, "Validation metrc:", metr)
        else:
            print("\nValidation loss:", total_losses / num_batch)
        self.log['valid']['loss'].append(total_losses / num_batch)
        ...
        if epoch % 5 == 0:
            ...
            self.save_checkpoint(epoch,
                                 save_optimizer=False,
                                 suffix=self.model_name + '_{}'.format(epoch))
        else:
            self.epochs_no_improve += 1
    ...
    def start_train(self):
        ...
        for epoch in range(self.start_epoch, self.max_epoch):
            ...
            if epoch > 0:
                if self.model_name == 'prolog':
                    if epoch % 5 == 0:
                        val_loss += self.valid_prolog(epoch)
                else:
