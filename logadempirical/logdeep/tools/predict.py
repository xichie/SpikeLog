...
class Predicter():
    ...
    def semi_supervised_helper(self, model, logs, vocab, data_type, scale=None, min_len=0):
        ...
        seqs, labels = sliding_window(sess_events, vocab, window_size=self.history_size, is_train=False,
                                      data_dir=self.data_dir, semantics=self.semantics, in_size=self.embedding_dim)
    ...
    def predict_semi_supervised(self):
        ...
        test_normal, test_abnormal = generate(self.output_dir, 'test.pkl', is_neural=False)
    ...
    def predict_pairwised(self):
        print("Loading vocab")
        with open(self.vocab_path, 'rb') as f:
            vocab = pickle.load(f)
        print(len(vocab))
        print("Loading train dataset for test.....\n")
        data = load_features(self.output_dir + "train.pkl", only_normal=False)
        ...
        test_data = load_features(self.output_dir + "test.pkl", only_normal=False)
        ...
        print("\nStarting phase: testing | ⏰: %s " % (start))
        total_losses = 0
        total_log = 0
        tbar = tqdm(self.test_loader, desc="\r")
        num_batch = len(self.test_loader)
        scores = []
        labels = []
        ...
        if total_log:
            print("\Testing loss:", total_losses / num_batch, "Testing metrc:", metr)
        else:
            print("\nTesting loss:", total_losses / num_batch)
